...
def permutation(block: bytearray, table: list, outlen: int):
...
def substitution(block: bytearray, table: list):
...
for round in range(16):
    ...
    for block_idx in range(8):
        substituted[4*block_idx:4*block_idx+4] = substitution(
            expansioned[6*block_idx:6*block_idx+6], S[block_idx])
    # Straight
...
assert(encrypted == DES.new(b"DreamCry", DES.MODE_ECB).encrypt(b"DEScrypt"))
print("[+]plain : {}".format(plain))
print("[+]crypt : {}".format(encrypted))
